...
export default {
  ...
  biobanks: ({ collectionInfo, biobankIds, biobanks }, { loading, rsql }) => {
    ...
    return ids.map(biobankId => {
      ...
      return {
        ...biobank,
        collections: filterCollectionTree(
          collectionInfo.map(it => it.collectionId),
          biobank.collections
        )
      }
    })
  ...
  getFoundCountries: (_, { biobanks }) => biobanks.map(b => b.country || 'not available'), // .filter(bid => bid !== undefined),
  ...
  foundCollectionIds (state, { getFoundBiobankIds }) {
    ...
    if (getFoundBiobankIds.length && state.collectionInfo.length) {
      ...
      for (const id of getFoundBiobankIds) {
        ...
        if (collectionsInBiobank) {
          collectionIds = collectionIds.concat(collectionsInBiobank)
        }
      }
